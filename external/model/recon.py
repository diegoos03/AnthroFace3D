...
class face_model:
    def __init__(self, args):
        ...
        model = np.load(ASSETS_DIR / "face_model.npy", allow_pickle=True).item()
        ...
        if args.extractTex:
            ...
            if self.device == 'cpu':
                from external.util.cpu_renderer import MeshRenderer_UV_cpu
                ...
                from external.util.nv_diffrast import MeshRenderer_UV
        ...
        if args.backbone == 'resnet50':
            ...
            self.net_recon.load_state_dict(torch.load(ASSETS_DIR / "net_recon.pth", map_location=torch.device('cpu'))['net_recon'])
        ...
        if args.backbone == 'mbnetv3':
            ...
            self.net_recon.load_state_dict(torch.load(ASSETS_DIR / "net_recon_mbnet.pth", map_location=torch.device('cpu'))['net_recon'])
            self.net_recon = self.net_recon.to(self.device)
            self.net_recon.eval()
